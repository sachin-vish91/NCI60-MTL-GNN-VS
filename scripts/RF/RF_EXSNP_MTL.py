...
a = pd.read_excel('Profile_data/EXSNP/DNA__Exome_Seq_AA_changing.xls', skiprows=10, header=0, na_values='-')
...
gexp[np.where(np.isnan(gexp))] = 0 # only in case of Exome SNP
...
x_train = np.loadtxt('/home/vishwakarma/Documents/GI50/NEW_DATASET/cell_line/'+cells_in_gexp[0]+'.train.csv', delimiter=',', dtype=np.float16)
y_train = np.loadtxt('/home/vishwakarma/Documents/GI50/NEW_DATASET/cell_line/'+cells_in_gexp[0]+'.train.act', dtype=np.float16)
x_train = np.hstack([x_train, np.tile(spearman_kernel[0], (len(x_train),1))])

for c in cells_in_gexp[1:]:
    t = np.loadtxt('/home/vishwakarma/Documents/GI50/NEW_DATASET/cell_line/'+c+'.train.csv', delimiter=',', dtype=np.float16)
...
mymodel = RandomForestRegressor(n_estimators=500, max_features=0.3, n_jobs = 10)
mymodel.fit(x_train, y_train)


for c in cells_in_gexp:
    t = np.loadtxt('/home/vishwakarma/Documents/GI50/NEW_DATASET/cell_line/'+c+'.test.csv', delimiter=',', dtype=np.float16)
    x_test = np.hstack([t, np.tile(spearman_kernel[cells_in_gexp.index(c)], (len(t),1))])
    y_test = np.loadtxt('/home/vishwakarma/Documents/GI50/NEW_DATASET/cell_line/'+c+'.test.act', dtype=np.float16)
    
    preds = mymodel.predict(x_test)
    
    with open('result_new/EXSNP/'+c+'.pred', 'w') as output:
